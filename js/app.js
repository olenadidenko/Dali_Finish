(() => {
    ...
    (() => {
        ...
        da.init();
        document.querySelectorAll(".menu-header__link").forEach((button => {
            button.addEventListener("click", (function(event) {
                document.querySelectorAll(".menu-header__item").forEach((item => {
                    if (item.querySelector(".menu-header__link") !== button) item.classList.remove("_sub-menu-active");
                }));
                event.isClick = true;
                button.parentElement.classList.toggle("_sub-menu-active");
            }));
        }));
        document.body.addEventListener("click", (function(event) {
            if (event.isClick == true || event.target.classList.contains("menu-header__link") == true || event.target.classList.contains("menu-header__sub-list") == true) return;
            document.querySelectorAll(".menu-header__item").forEach((item => {
                item.classList.remove("_sub-menu-active");
            }));
        }));
        document.querySelectorAll(".column2-footer__title").forEach((button => {
            button.addEventListener("click", (function(event) {
                button.parentElement.classList.toggle("column2-footer__item_open");
            }));
        }));
        window["FLS"] = true;
        isWebp();
